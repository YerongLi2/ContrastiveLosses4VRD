...
def eval_rel_results(all_results, output_dir, do_val):
    ...
    for phrdet in eval_sets:
        ...
        for prd_k in prd_k_set:
            ...
            ############################### added by Wentao
            # store each relation's times of being recalled
            recall_20_count_50_rel_types = np.zeros(50)
            recall_50_count_50_rel_types = np.zeros(50)
            recall_100_count_50_rel_types = np.zeros(50)
            ...
            for im_i, res in enumerate(tqdm(all_results[:2000])):
                ...
                if do_val:
                    ...
                    for k in recalls:
                        ...
                        ############################### added by Wentao
                        if prd_k == 20: # only do statistics on prd_k == 20
                            if k==20:
                                for label_num in gt_labels_prd:
                                    recall_20_count_50_rel_types[label_num] += 1
                                for label_idx_num in match:
                                    if type(label_idx_num) == int:
                                        recall_20_count_50_rel_types[gt_labels_prd[label_idx_num]] -= 1
                                    else:
                                        recall_20_count_50_rel_types[gt_labels_prd[label_idx_num.astype(int)]] -= 1
                            if k==50:
                                for label_num in gt_labels_prd:
                                    recall_50_count_50_rel_types[label_num] += 1
                                for label_idx_num in match:
                                    if type(label_idx_num) == int:
                                        recall_50_count_50_rel_types[gt_labels_prd[label_idx_num]] -= 1
                                    else:
                                        recall_50_count_50_rel_types[gt_labels_prd[label_idx_num.astype(int)]] -= 1
                            if k==100:
                                for label_num in gt_labels_prd:
                                    recall_100_count_50_rel_types[label_num] += 1
                                for label_idx_num in match:
                                    if type(label_idx_num) == int:
                                        recall_100_count_50_rel_types[gt_labels_prd[label_idx_num]] -= 1
                                    else:
                                        recall_100_count_50_rel_types[gt_labels_prd[label_idx_num.astype(int)]] -= 1
            ...
            if do_val:
                ...
                if prd_k == 20: # only do statistics on prd_k == 20
                    ...
                    # Statistics for R@20
                    x = np.arange(det_labels_prd.shape[1])
                    y = recall_20_count_50_rel_types
                    tick_label = []
                    for i in range(det_labels_prd.shape[1]):
                    ...
                    plt.title("Statistics of R@20")
                    plt.xlabel("Label number")
                    plt.ylabel("Times not being recalled")
                    # plt.grid()
                    # plt.show()
                    plt.savefig('R@20_Statistics.png', format='png')


                    # Statistics for R@50
                    y = recall_50_count_50_rel_types
                    # bar_width = 0.3
                    plt.clf()
                    plt.rcParams['figure.dpi'] = 300
                    plt.rcParams['savefig.dpi'] = 300
                    ...
                    # plt.bar(x, y, tick_label=tick_label, width=bar_width)
                    plt.bar(x, y, tick_label=tick_label)
                    plt.title("Statistics of R@50")
                    plt.xlabel("Label number")
                    plt.ylabel("Times not being recalled")
                    # plt.grid()
                    # plt.show()
                    plt.savefig('R@50_Statistics.png', format='png')


                    # Statistics for R@100
                    y = recall_100_count_50_rel_types
                    # bar_width = 0.3
                    plt.clf()
                    plt.rcParams['figure.dpi'] = 300
                    plt.rcParams['savefig.dpi'] = 300
                    ...
                    # plt.bar(x, y, tick_label=tick_label, width=bar_width)
                    plt.bar(x, y, tick_label=tick_label)
                    plt.title("Statistics of R@100")
                    plt.xlabel("Label number")
                    plt.ylabel("Times not being recalled")
                    # plt.grid()
                    # plt.show()
                    plt.savefig('R@100_Statistics.png', format='png')
